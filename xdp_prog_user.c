			...
			struct stats_record *stats_prev)
{
	...
	/* Assignment#2: Print other XDP actions stats  */
	{
		...
		printf(fmt, action, rec->total.rx_packets, pps,rec->total.rx_bytes, period);
	}
}
...
int main(int argc, char **argv)
{
	...
	while (true)
	{
		...
		for (int  i = 0; i < 5; i++)
		{
			int res = bpf_map_lookup_elem(bpf_map__fd(map),&i,&value);
			if(res!=0){
				fprintf(stderr,"Failure in lookup \n");
				return EXIT_FAILURE;
			} 
			char *name = printKeyID(i);
			printf("%d key %s : Packet Count %llu, Byte count: %llu \n",i,name,value.rx_packets,value.rx_bytes);
		}
		printf("\n\n");
		sleep(5);
	}
